...
namespace nacl
{
  public static class SafeComparison
  {
    ...
    {
      return Verify16((ArraySegment<byte>)x, (ArraySegment<byte>)y);
    }

    public static bool Verify16(byte[] x, int xOffset, byte[] y, int yOffset)
    {
      return Verify16((ArraySegment<byte>)x + xOffset, (ArraySegment<byte>)y + yOffset);
    }

    internal static bool Verify16(ArraySegment<byte> x, ArraySegment<byte> y)
    {
    ...
    {
      return Verify32((ArraySegment<byte>)x, (ArraySegment<byte>)y);
    }

    public static bool Verify32(byte[] x, int xOffset, byte[] y, int yOffset)
    {
      return Verify32((ArraySegment<byte>)x + xOffset, (ArraySegment<byte>)y + yOffset);
    }

    internal static bool Verify32(ArraySegment<byte> x, ArraySegment<byte> y)
    {
      uint differentBits = 0;
